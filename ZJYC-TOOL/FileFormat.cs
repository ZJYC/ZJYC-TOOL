...
namespace ZJYC_TOOL
{
    class FileFormat
    {
        ...
        public void Hex2BinWithSameName(string FileNameInput)
        {
            Hex2Bin hex2Bin = new Hex2Bin();
            byte[] Temp = hex2Bin.ReadHexFileToByte(FileNameInput);
            string Path = System.IO.Path.GetDirectoryName(FileNameInput);
            Path += "\\" + System.IO.Path.GetFileNameWithoutExtension(FileNameInput) + ".bin";
            hex2Bin.WritByteToFile(Temp,Path);
        }
    }
}
